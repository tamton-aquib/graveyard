...
fn render_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let (start_y, end_y, start_x, end_x) = app.viewport.visible_range(
        app.total_virtual_rows(),
        app.handler.total_cols()
    );
    ...
    for r in start_y..end_y {
        ...
        cells.push(Cell::from((app.actual_row(r) + 1).to_string()).style(Style::default().fg(Color::DarkGray)));
...
fn render_status_bar(f: &mut Frame, app: &mut App, area: Rect) {
    let mode_str = match app.mode {
        AppMode::Normal => format!("NORMAL | Sheet: {} [{}/{}] | Cell: ({}, {})", 
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title(ratatui::widgets::block::Title::from(" 🦀 ").alignment(ratatui::layout::Alignment::Right))
    );
